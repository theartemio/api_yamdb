...
def upload_csv_data(path_to_file, current_model, model_name):
    """Загружает данные из .csv в модель."""
    to_create = []
    mtm_middle_models = ['title_genre']
    with open(path_to_file, encoding='utf-8') as csv_file:
        ...
        for line in reader:
            for field_name, field_value in line.items():
                ...
                print(current_model)
                if model_name not in mtm_middle_models and isinstance(field, models.ForeignKey):
